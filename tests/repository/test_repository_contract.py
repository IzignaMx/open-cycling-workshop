...
def test_web_workspace_wires_generated_api_client_and_same_origin_dev_proxy() -> None:
    ...
    assert tsconfig['compilerOptions']['paths']['@ocwp/api-client'] == ['../../packages/api-client/src/index.ts']
...
def test_hosted_browser_e2e_uses_the_production_pwa_build() -> None:
    ...
    assert "process.env.CI ? 'pnpm exec vite preview --host 127.0.0.1 --port 5173'" in config
