...
def test_probe_tool_requires_the_executable_to_run_successfully() -> None:
    ...
    failing = MODULE.probe_tool('broken', sys.executable, ['-c', 'raise SystemExit(3)'], 'tool cannot execute')
...
def test_indexeddb_runtime_check_preserves_environment_blockers(monkeypatch) -> None:
    class Result:
        returncode = 2
        stdout = '{"status":"blocked","reason":"chromium managed URLBlocklist blocks all navigations"}'
        stderr = ''
...
def test_static_analysis_checks_run_available_python_and_frontend_tools(monkeypatch) -> None:
    ...
    assert ('python_ruff', ('ruff', 'check', 'services/platform/src', 'services/platform/tests', 'tests', 'scripts')) in calls
    assert any(name == 'frontend_eslint' and command[:3] == ('pnpm', 'exec', 'eslint') for name, command in calls)


def test_static_analysis_checks_report_blocked_groups_without_executing_commands(monkeypatch) -> None:
    ...
    assert checks == []
    assert {blocker.name for blocker in blockers} == {'python_static_analysis', 'frontend_static_analysis'}
...
def test_postgresql_runtime_checks_block_when_driver_or_database_is_unavailable(monkeypatch) -> None:
...
def test_postgresql_runtime_checks_execute_integration_when_ready(monkeypatch) -> None:
    ...
    checks, blockers = MODULE.postgresql_runtime_checks({'OCWP_TEST_DATABASE_URL': 'postgresql://example.invalid/ocwp_e2e'})

    assert blockers == []
    assert {item.name for item in checks} == {'python_psycopg', 'postgresql_integration'}
    assert any(name == 'postgresql_integration' and 'test_postgres_integration.py' in ' '.join(command) for name, command in calls)
