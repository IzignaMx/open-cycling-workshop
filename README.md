...
> Status: **V0.1 Foundations / implementation checkpoint H (GitHub qualification)**. This repository is not yet a production release.
...
The verifier distinguishes failures from environment-dependent blockers. In this execution environment the Python suite, real Uvicorn HTTP smoke, TypeScript core tests, generated API client tests, fallback frontend typecheck, repository contract and OpenAPI drift checks can run. The local execution container cannot reach npm/PyPI registries and has browser/network policy restrictions, so those gates remain incomplete locally. GitHub Actions is the authorized hosted qualification environment for dependency resolution, full static analysis, PostgreSQL 18.4, production PWA build and Playwright + axe evidence.
...
3. `docs/10-spec-development/execution-state.yaml`
4. `docs/superpowers/plans/2026-08-07-v0.1-bootstrap-customer-slice.md`

## Current blockers

The repository intentionally does not fake evidence for tools unavailable in the execution environment. `pnpm-lock.yaml`, `uv.lock`, real Docker/PostgreSQL smoke tests, browser IndexedDB tests, Android/Windows storage qualification and hosted security scans remain pending until they can be executed against their real runtimes.
